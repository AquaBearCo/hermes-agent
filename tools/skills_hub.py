...
class WellKnownSkillSource(SkillSource):
    ...
    def _parse_index(self, index_url: str) -> Optional[dict]:
        cache_key = f"well_known_index_{internal_digest(index_url.encode())}"
...
class SkillsShSource(SkillSource):
    ...
    def search(self, query: str, limit: int = 10) -> List[SkillMeta]:
        ...
        cache_key = f"skills_sh_search_{internal_digest(f'{query}|{limit}'.encode())}"
        cached = _read_index_cache(cache_key)
        if cached is not None:
    ...
    def _fetch_detail_page(self, identifier: str) -> Optional[dict]:
        cache_key = f"skills_sh_detail_{internal_digest(identifier.encode())}"
        cached = _read_index_cache(cache_key)
...
class ClawHubSource(SkillSource):
    ...
    def search(self, query: str, limit: int = 10) -> List[SkillMeta]:
        ...
        cache_key = f"clawhub_search_listing_v1_{internal_digest(query.encode())}_{limit}"
        cached = _read_index_cache(cache_key)
        if cached is not None:
    ...
    def _search_catalog(self, query: str, limit: int = 10) -> List[SkillMeta]:
        cache_key = f"clawhub_search_catalog_v1_{internal_digest(f'{query}|{limit}'.encode())}"
        cached = _read_index_cache(cache_key)
        if cached is not None:
